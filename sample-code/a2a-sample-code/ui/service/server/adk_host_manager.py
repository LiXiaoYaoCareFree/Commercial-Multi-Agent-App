...
class ADKHostManager(ApplicationManager):
    ...
    def __init__(
        ...
        self._host_agent = HostAgent(
            [], http_client or httpx.AsyncClient(), self.task_callback
        )
        ...
        try:
            ...
            if not self._host_runner:
                response = Message(
                    parts=[
                        Part(
                            root=TextPart(
                                text='未配置模型认证信息：请在 Settings 填写 GOOGLE_API_KEY，或设置 GOOGLE_GENAI_USE_VERTEXAI=TRUE 后重启。'
                            )
                        )
                    ],
                    role=Role.agent,
            ...
            response = Message(
                parts=[
                    Part(
                        root=TextPart(
                            text='模型调用超时（60s）。请检查网络/代理、API Key、或更换更快的模型。'
                        )
                    )
                ],
                role=Role.agent,
            ...
            response = Message(
                parts=[
                    Part(
                        root=TextPart(
                            text=f'处理消息失败：{type(e).__name__}: {e}'
                        )
                    )
                ],
                role=Role.agent,
    ...
    async def adk_content_to_message(
        ...
        role = (
            ...
            or str(content.role).lower() == 'user'
            else Role.agent
        )
